# Récupère les résultats du serveur, conserve les waggle_phases et vérifie le lot avant d'autoriser le nettoyage distant.
...
colonnes_tri = [c for c in ["date", "heure", "nom_fichier"] if c in res.columns]
if colonnes_tri:
    res = res.sort_values(colonnes_tri)
...
videos_avec_phases = res.loc[res["nb_phases_WPM"] > 0, "nom_fichier"].astype(str)
fichiers_waggle = list(temp_waggle.rglob("waggle_phases.csv"))
dossiers_recuperes = {dossier_video(f) for f in fichiers_waggle}

manquants = [nom for nom in videos_avec_phases if Path(nom).stem not in dossiers_recuperes]
if manquants:
...
for fichier in fichiers_waggle:
    ...
    if nom_video is None:
        raise RuntimeError(f"Nom vidéo introuvable : {fichier}")

    correspondance = df_lot[df_lot["nom_fichier"].astype(str).map(lambda x: Path(x).stem) == nom_video]
    if len(correspondance) != 1:
        raise RuntimeError(f"Impossible d'identifier la vidéo : {nom_video}")
...
nb_phases_resume = int(res["nb_phases_WPM"].sum())

if waggle_lot:
    waggle_lot = pd.concat(waggle_lot, ignore_index=True)
    waggle_lot.to_csv(csv_waggle_lot, sep=";", index=False, encoding="utf-8-sig")
    nb_phases_detail = len(waggle_lot)
else:
    nb_phases_detail = 0
    pd.DataFrame(columns=[
        "ruche", "nom_fichier", "chemin", "date", "heure",
        "categorie_traitement", "hyperparametre_file", "nom_lot"
    ]).to_csv(csv_waggle_lot, sep=";", index=False, encoding="utf-8-sig")

if nb_phases_detail != nb_phases_resume:
    raise RuntimeError("Incohérence entre nb_phases_WPM et les waggle_phases récupérés.")

shutil.rmtree(temp_waggle)

fichier_ok.write_text(
    f"lot={nom_lot}\nruche={ruche_courte}\nvideos={len(res)}\nphases={nb_phases_detail}\n",
    encoding="utf-8",
)

(tmp / "recuperation_ok.txt").write_text(
    f"{nom_lot}\n{len(res)} vidéos\n{len(fichiers_waggle)} fichiers waggle_phases\n{nb_phases_detail} phases détaillées\n",
    encoding="utf-8",
)

print("Vidéos récupérées :", len(res))
print("Fichiers waggle_phases :", len(fichiers_waggle))
print("Phases détaillées :", nb_phases_detail)
print("Lot validé :", fichier_ok)

...
wpm = os.environ.get("WPM_COMMAND", "wpm")
lat = "46.153895"
lon = "-0.689021"
tz = "Europe/Paris"
...
def clean_output(out):
    garder = {"waggle_phases.csv", "detection.log", "batch_log.txt"}
    for f in out.rglob("*"):
        if f.is_file() and f.name not in garder:
            try:
                f.unlink()
            except OSError:
                pass
...
def run_video(row, passage, timeout_s, delay=0):
    ...
    cmd = ["timeout", str(timeout_s), wpm, "detection", "--detection__min_speed", min_speed, "--output_path", str(out), "--hyperparameters_file_path", str(hyperparam), "--", str(video), lat, lon, tz]
    t0 = time.time()
    ...
    resultat.update({"passage": passage, "statut": statut, "nb_phases_WPM": nb, "temps_s": temps, "chemin_serveur": str(video), "output_path": str(out)})
    return resultat
...
with ThreadPoolExecutor(max_workers=n_parallel) as ex:
    futures = [ex.submit(run_video, row.copy(), "passage_1", timeout_1, (k % n_parallel) * delai_lancement) for k, (_, row) in enumerate(df.iterrows())]
    for i, future in enumerate(as_completed(futures), 1):
        r = future.result()
        res1.append(r)
        log(f"[{i}/{len(df)}] {r['statut']} | {r['temps_s']} s | {r['nb_phases_WPM']} phases | {r['nom_fichier']}")
        pd.DataFrame(res1).to_csv(status_csv, sep=";", index=False, encoding="utf-8-sig")

retry = [r for r in res1 if r["statut"] in {"echec_timeout", "echec_detection_slow", "echec"}]
log(f"Passage 2 : {len(retry)} vidéos à relancer")
res2 = []
if retry:
    with ThreadPoolExecutor(max_workers=n_parallel_retry) as ex:
        futures = [ex.submit(run_video, pd.Series(r), "passage_2", timeout_2, (k % n_parallel_retry) * delai_lancement) for k, r in enumerate(retry)]
        for i, future in enumerate(as_completed(futures), 1):
            r = future.result()
            res2.append(r)
            log(f"[retry {i}/{len(retry)}] {r['statut']} | {r['temps_s']} s | {r['nb_phases_WPM']} phases | {r['nom_fichier']}")

retry_nom = {str(r["nom_fichier"]): r for r in res2}
final = []
for r1 in res1:
    r2 = retry_nom.get(str(r1["nom_fichier"]))
    r = (r2 if r2 is not None else r1).copy()
    t2 = float(r2["temps_s"]) if r2 is not None else 0.0
    r["temps_passage_1_s"] = float(r1["temps_s"])
    r["temps_passage_2_s"] = t2
    r["a_ete_relancee"] = r2 is not None
    r["temps_total_video_s"] = round(float(r1["temps_s"]) + t2, 1)
    final.append(r)

final = pd.DataFrame(final).sort_values("nom_fichier")
